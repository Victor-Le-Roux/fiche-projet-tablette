...
function validateForm(form) {
  ...
  if (isBlank(form.requestDate)) errors.requestDate = 'Date de demande obligatoire.';
  if (isBlank(form.organizers)) errors.organizers = 'Nom du/des organisateur(s) obligatoire.';
  if (isBlank(form.activityName)) errors.activityName = "Nom de l'activité obligatoire.";
  if (form.categories.length === 0) {
    errors.categories = 'Sélectionnez au moins une catégorie.';
  }
  if (form.categories.includes('Autres') && isBlank(form.categoryOther)) {
    errors.categoryOther = 'Précisez l’autre catégorie.';
  }
  if (!isMaterialPurchase && form.goals.length === 0) {
    errors.goals = 'Sélectionnez au moins un but.';
  }
  if (!isMaterialPurchase && form.goals.includes('Autres') && isBlank(form.goalOther)) {
    errors.goalOther = 'Précisez l’autre but.';
  }
  if (isBlank(form.participantsCount)) {
    errors.participantsCount = 'Nombre de participants obligatoire.';
  }
  if (!form.facebookCommitment) {
    errors.facebookCommitment = 'Engagement de diffusion obligatoire.';
  }
  if (!form.receiptsCommitment) {
    errors.receiptsCommitment = 'Engagement sur les justificatifs obligatoire.';
  }
...
function buildPrintHtml(form, totals) {
  ...
  const firstDateText = isBlank(form.firstDate) ? 'à définir' : form.firstDate;
  ...
  const needsRows = annotateNeedDisplayLabels(printableNeeds)
    .filter(({ need }) => shouldPrintNeed(need))
  ...
  <body>
    <main class="print-page">
      <header class="header">
        ...
        <div class="validity">
          ${hasRepetitions ? 'Coût 3 mois' : 'Coût estimé'}<br>
      ...
      <div class="info-grid">
        ...
        <section>
          ...
          <table class="two-col">
            <tr><td>Date demande</td><td>${textValue(form.requestDate)}</td></tr>
            ...
            <tr><td>Commentaires</td><td>${textValue(form.comments)}</td></tr>
          </table>
        </section>
      </div>
...
export default function App() {
  ...
  function updateMealLocation(value) {
    setForm((current) => ({
      ...
      needs: current.needs.map((need) =>
        need.label === 'Repas' ? { ...need, estimatedCost: mealPrice(value) } : need,
      ),
    }));
  }
    ...
    setForm((current) => ({
      ...
      needs: normalizeManualRepeatableNeeds(
        current.needs.map((need, needIndex) =>
          needIndex === index ? { ...need, [key]: value } : need,
        ),
      ),
    }));
  }
  ...
  function renderNeedRow({
    ...
    const showDetail = hasQuantityField && hasMinimumQuantityForDetail(need);
    const hasCostField = hasEstimatedCostField(need);
    const canCancelPaidActivity = isPaidActivity && isNeedFilled(need);
      ...
      <div className={rowClassName}>
        ...
        {hasCostField && (
          <input
            ...
            aria-label={`${need.label} coût estimé`}
          />
        )}
        {canCancelPaidActivity && (
          <button
            ...
            aria-label={`Annuler ${displayLabel}`}
          >
            Annuler
          </button>
        )}
  ...
  function resetForm() {
    const confirmed = window.confirm('Effacer cette fiche et commencer une nouvelle ?');
    if (!confirmed) return;
    startNewForm();
  }
  ...
  async function printForm() {
    ...
    try {
      ...
      // Le retour accueil reste prioritaire pour éviter de réutiliser la fiche par erreur.
    } finally {
      returnToHome("L'impression est terminée ou fermée.");
    }
  }
        ...
        <div className="section-heading">
          ...
          <div className="totals">
            <span className={`budget-total ${budgetLevelClassName(totals.estimated)}`}>
              Estimé : {formatCurrency(totals.estimated)}
            </span>
            {hasRepetitions && (
              <span className={`budget-total ${budgetLevelClassName(projectedCost)}`}>
                3 mois : {formatCurrency(projectedCost)}
              </span>
            )}
          </div>
        </div>
          ...
          <div className="summary-grid">
            <div className="summary-panel">
              ...
              <dl>
                ...
                <div>
                  <dt>1ère date</dt>
                  <dd>{form.firstDate || 'à définir'}</dd>
                </div>
                <div>
                  <dt>Participants</dt>
